...
# this is specialized to subdiagonal for now
def resolvent_bilinear_flattened_batched_(subd, v, u, m, d, S):
    # pass at depth d computes 4 arrays:
    # each array is length n, indexed by x_{m-1}, ..., x_{m-d}, y_{m-d-1}, ..., y_0
    # for convenience, store as x_{m-d}, ..., x_{m-1}, y_{m-d-1}, ..., y_0
    ...
    # S0_10[:n/2], S1_10[:n/2] = S_10[:n/2], S_10[n/2:]
    # S0_11[:n/2], S1_11[:n/2] = S_11[:n/2], S_11[n/2:]
    S_storage = np.zeros((4, n1, n2))
    S0_10, S0_11, S1_01, S1_11 = S_storage
    S0_10[:,:n2//2] = S_10[:n1,:]
    S1_01[:,:n2//2] = S_01[n1:,:]
    S0_11[:,:n2//2] = S_11[:n1,:]
    S1_11[:,:n2//2] = S_11[n1:,:]
    ...
    storage_fft = np.fft.rfft(S_storage)
    S0_10_, S0_11_, S1_01_, S1_11_ = storage_fft
    ...
    T_storage = np.stack((S1_01_ * S0_10_, S1_01_ * S0_11_, S1_11_ * S0_10_, S1_11_ * S0_11_))
    T_storage = A_subd * np.fft.irfft(T_storage)
    T_00, T_01, T_10, T_11 = T_storage
...
def resolvent_bilinear_flattened_batched(A, v, u, n, m):
    assert n == 1<<m # power of 2 for now
    ...
    for d in range(m-1,-1,-1):
        S = resolvent_bilinear_flattened_batched_(subd, v, u, m, d, S)
