...
__global__ void subdiagMult(float *d_Subdiag, float *d_Data, float *d_Output, int N) {
    int        tid = blockIdx.x * blockDim.x + threadIdx.x;
    int numThreads = blockDim.x * gridDim.x;

    for (int pos = tid; pos < N; pos += numThreads)
    {
        // d_Output[pos] = d_Subdiag[pos] * d_Data[pos != 0 ? pos - 1 : N - 1];
        d_Output[pos] = d_Subdiag[pos] * d_Data[(pos - 1 + N) % N];
    }
}

void subdiagMultGPU(float *d_Subdiag, float *d_Data, float *d_Output, int N) {
    const int THREAD_N = 256;
    subdiagMult<<<(N + THREAD_N - 1) / THREAD_N, THREAD_N>>>(d_Subdiag, d_Data, d_Output, N);
}